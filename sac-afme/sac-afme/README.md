...
- **POMDP**: obs=최근 L=10 혁신노름(EMA 정규화), action tanh 매핑(RL-AKF식 범위 제약),
  reward r=−min(100·‖p−p̂‖², 100), done=0(무한지평 절단), 동기화 M=64 벡터 에피소드
  (워밍업 8 + RL 400 스텝), log-replay(측정노이즈 σ∈[0.03,0.10] 온라인 주입 = 증강).
