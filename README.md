...
**급기동(high-G)은 의도적으로 배제** — 12상태 동역학의 1차 테일러 선형화가 무너지는
영역이라 (N,λ) 적응의 범위 밖. 대신 문헌이 검증한 **급격 이벤트 + 회복** 시나리오를
이식: payload 급결합(UIFM-SLAC Sc.2), NLOS 완전 dropout(UIFM-SLAC Sc.3/DI-FME),
급임펄스 돌풍(FM-SMC). 근거는 1_FIR_filter Fig.3/5의 UFIR transient-noise 트레이드오프
(작은 N=과도 짧게, 큰 N=노이즈 평균화)이며, N_opt(t)를 SAC가 추적한다.

**진단(합성 heldout 16궤적, `tools/diagnose_recovery.py`)** — 적응 여지 확인:
| 외란 구간 | best-fixed | Greedy(상한) | 적응 여지 |
|---|---|---|---|
| payload 급결합 | 0.153 | 0.119 | 22.2% |
| 급임펄스 돌풍 | 1.388 | 0.881 | 36.5% |
| NLOS dropout | 1.479 | 1.103 | 25.4% |
| **전체 외란** | 0.857 | 0.639 | **25.4%** |

dropout의 큰 오차는 GDOP 악화(3앵커, 워크스페이스 모서리 GDOP~11)로 물리적으로 정당 —
큰 N이 dropout 이전 4앵커 데이터를 붙들어 버티는 상황이 (N,λ) 적응의 최명확 사례.
